...
> 
> 这段话的关键，不是“专用数据库一定比 Postgres 高级”，而是要把“性能最优解”和“工程默认起点”分开看。Stonebraker 说 `one size fits none`，意思是不同 workload 在底层根本不是同一种问题：交易系统关心的是小读写、锁、事务与一致性；分析系统关心的是列存、压缩、扫描和聚合；向量检索关心的是近似最近邻索引；流处理系统关心的是持续事件、窗口和低延迟反应。你可以用通用数据库去做这些事，但它通常不会是最优执行器。
> 
> 他举的几个系统，正好对应不同负载的“为场景而生”。StreamBase 这类系统按流式事件处理设计；Vertica 和 ClickHouse 按分析型查询、列式存储和批量扫描设计；Pinecone 按向量召回与近似最近邻检索设计。它们之所以快，不只是“实现更好”，而是数据布局、索引结构、执行模型和缓存策略从第一天就是围绕那种负载定制的，所以一旦场景匹配，性能差距可能真的是一个数量级。
> 
> 但这不等于团队应该一开始就把系统拆成一堆专用数据库。Stonebraker 给 Postgres 的现实定位是：如果你的问题还没极端到百万 TPS 或 PB 级数仓，真正需要的往往不是单项性能冠军，而是一个事务可靠、SQL 成熟、生态完备、运维成本可控的默认底座。对订单、支付、CRM、内部后台、Agent workflow 元数据这类系统来说，Postgres 经常不是最极致，却是最稳妥的起点。
> 
> 更实用的理解是两阶段路线：先用 Postgres 把产品闭环和真实负载跑出来，再根据瓶颈决定是否引入专用系统。如果将来主要压力变成大规模分析，再考虑 ClickHouse 或 Vertica；如果变成向量召回，再考虑 Pinecone 一类系统；如果只是中早期业务系统，过早拆库通常不是被性能救了，而是先被复杂度拖住。这也是他那句“默认选 Postgres 仍然非常合理”真正想表达的工程判断。
...
> [!info]+ 💡 Explanation - Hadoop, Eventual Consistency, and Why Spanner Matters
> 
> 这里先要分清一个历史语境：Stonebraker 口中的 “Google 路线”，严格说指的是 Google 当年用论文推广出来的 MapReduce / GFS / Bigtable 那一套分布式大数据思路；很多人后来会直接用 Hadoop 来代称这条路线，因为 Hadoop 是 Apache 按这套思路做出来的开源实现和生态。它擅长的是大规模离线批处理，例如日志分析、ETL、索引构建、按天跑的大任务，而不是高事务、高一致性的在线数据库问题。
> 
> Hadoop / MapReduce 的基本逻辑是：先把大数据集切分到很多机器上，本地做一轮 map，再把中间结果经过网络重新分发、排序和聚合，最后 reduce 出结果。对“扫描海量文件、慢慢算出一个大结果”这类工作，它很有价值；但在 Stonebraker 看来，很多原本可以由数据库高效完成的查询、聚合和 join，被这套路拆成了过于笨重的批处理流程，所以效率很差。批量扫描、反复落盘、网络搬运、再聚合，这些成本在数据库世界里通常会被更紧凑地处理。
> 
> eventual consistency 则是另一层问题。它的意思不是系统永远错，而是允许多个副本暂时不一致，之后再异步收敛。在少数容忍短时误差的场景里，这样可以换来更高可用性和更简单的跨地域扩展；但如果你处理的是库存、订单、账户余额这种强约束状态，代价就会非常直接。Stonebraker 拿东西海岸同时卖最后一件商品举例，就是为了说明：如果两个地方都先看到“还有货”，然后事后再对账，最终就会有一笔卖单必须被推翻。
> 
> Spanner 之所以重要，是因为它代表 Google 后来自己承认了这个问题不能靠“以后再收敛”来绕开。Spanner 不是 Hadoop 那种离线批处理框架，而是全球分布式数据库：目标是在跨机房、跨地域部署下，依然尽量保住强一致性、事务和可靠提交顺序。它的意义不在于“Google 又造了一个更复杂的系统”，而在于它重新把数据库最老的问题搬回了分布式时代的中心：多个地方同时写同一份世界状态时，谁先提交，谁后提交，冲突怎么解决，系统崩了之后到底哪笔写入算数。
> 
> 所以 Stonebraker 这里真正想说的是：Hadoop 代表“先把大数据算出来”的离线思路，Spanner 代表“必须把真实业务状态保持正确”的在线数据库思路。前者适合大规模批处理，后者适合全球分布式但仍然要求事务正确性的业务系统。Google 后来愿意做 Spanner，本身就说明在高价值系统里，事务和一致性最终还是绕不过去。
