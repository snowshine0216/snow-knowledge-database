...
> #### Q1 — 真正稀缺的工程理解能力
> 
> 真正稀缺的是能理解框架底层机制、设计和优化生产级 Agent 系统的人。仅停留在 API 调用层面的人无法理解为什么某个设计是这样而不是那样，也无法在遇到问题时知道该从哪里下手——例如消息进入 Agent 到触发工具调用的八个处理站点、Agent Loop 的三层架构协作关系，都需要源码级理解才能掌握。
> 
> #### Q2 — OpenClaw 三个关键工程洞察
> 
> 三个理由：
> 
> ① **足够真实** — OpenClaw 是可部署的生产级系统（33 种 LLM、21 种消息通道），面对真实世界复杂性，暴露 Demo 级框架看不到的工程挑战；
> 
> ② **架构精妙** — Agent Loop 仅用 run.ts（外层重试循环）和 attempt.ts（单次尝试）两个文件实现七重容错与事件驱动流式处理，体现"在不确定性中寻找确定性"的哲学；
> 
> ③ **Local-First** — 数据主权归用户，本地存储与向量搜索，是隐私价值观的体现，也代表 Agent 发展的重要方向。
> 
> #### Q3 — 三个大坑与系统化解决方案
> 
> 三个坑及解决方案：
> 
> ① **LLM 调用超时导致 Agent 卡死** → 需要超时重试、模型降级、上下文恢复的七重容错策略；
> 
> ② **对话半小时后 Token 超限、上下文全丢** → 需要从启动前检查到运行时截断的五层纵深上下文防护；
> 
> ③ **用户输入嵌入 Prompt 注入指令险些执行危险 bash 命令** → 需要七层纵深安全架构，安全是 Agent 从实验室玩具走向生产部署的生死线。
