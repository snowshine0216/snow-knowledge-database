...
1. "VibeCoding"这个概念意味着什么？它对 AI Agent 开发者的技能要求会产生怎样的影响——是让门槛更低，还是让深度理解变得更重要？
2. 如果你要把一个 AI Agent 部署到生产环境，你预测会遇到哪些工程挑战（比如稳定性、安全性、上下文管理等）？
3. 市面上有 LangGraph、AutoGen、CrewAI、Dify 等多个 Agent 框架，你会用什么标准来选择一个框架作为深度学习的对象？
...
1. 作者说"AI Agent 时代最稀缺的不是会用框架的人"——用自己的话解释，真正稀缺的是什么样的能力，以及为什么"会调 API、写 Prompt、跑 Demo"还不够？
2. 作者选择 OpenClaw 作为课程载体有三个关键理由，请逐一解释每个理由背后的工程洞察（比如 run.ts/attempt.ts 的设计说明了什么？Local-First 哲学代表了什么价值取向？）。
3. 作者在 Agent 开发中踩过三个大坑，请分别描述每个坑的具体场景，以及 OpenClaw 提供了怎样的系统化解决方案来应对每种风险。

<details>
<summary>答案指南</summary>

1. 真正稀缺的是能理解框架底层机制、设计和优化生产级 Agent 系统的人。仅停留在 API 调用层面的人无法理解为什么某个设计是这样而不是那样，也无法在遇到问题时知道该从哪里下手——例如消息进入 Agent 到触发工具调用的八个处理站点、Agent Loop 的三层架构协作关系，都需要源码级理解才能掌握。
2. 三个理由：①足够真实——OpenClaw 是可部署的生产级系统（33 种 LLM、21 种消息通道），面对真实世界复杂性，暴露 Demo 级框架看不到的工程挑战；②架构精妙——Agent Loop 仅用 run.ts（外层重试循环）和 attempt.ts（单次尝试）两个文件实现七重容错与事件驱动流式处理，体现"在不确定性中寻找确定性"的哲学；③Local-First——数据主权归用户，本地存储与向量搜索，是隐私价值观的体现，也代表 Agent 发展的重要方向。
3. 三个坑及解决方案：①LLM 调用超时导致 Agent 卡死→需要超时重试、模型降级、上下文恢复的七重容错策略；②对话半小时后 Token 超限、上下文全丢→需要从启动前检查到运行时截断的五层纵深上下文防护；③用户输入嵌入 Prompt 注入指令险些执行危险 bash 命令→需要七层纵深安全架构，安全是 Agent 从实验室玩具走向生产部署的生死线。
