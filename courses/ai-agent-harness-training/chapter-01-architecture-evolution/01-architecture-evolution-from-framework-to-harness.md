...
1. DAG 图的逻辑是硬编码的：若 NodeA 执行时遇到网络超时或意外 JSON 格式，框架没有弹性回退机制，直接抛异常崩溃；底层隐式状态机一旦进入死循环，开发者无法从外部插手干预。
2. 控制反转指：业务流程的执行顺序不再由 Go/Python 代码决定，而是完全交由大模型实时推理和规划；代码从"业务逻辑编排者"变为"物理定律提供者"（只提供文件读写、bash 执行等原子能力）。
3. 阶梯压缩器类比 OS 的内存回收器（Memory Compactor / GC + Swap）；接近上限时应分级响应：先摘要早期对话、再删除冗余轮次、再将历史 Swap 到本地文件，确保 Agent 不会因 API 超限而彻底失忆崩溃。
