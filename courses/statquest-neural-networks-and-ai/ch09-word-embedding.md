...
1. An embedding table is a weight matrix of shape `[vocab_size × embedding_dim]`; multiplying a one-hot vector by this matrix selects (looks up) the row corresponding to that word, producing a dense low-dimensional vector — equivalent to an `nn.Embedding` lookup in PyTorch.
2. Skip-Gram takes a center word as input and tries to predict its surrounding context words; the network is trained with cross-entropy loss, and the learned input weights become the word embeddings used downstream.
3. Full softmax over a large vocabulary is computationally expensive; Negative Sampling replaces it by training a binary classifier — the model learns to distinguish the true context word from a small set of randomly sampled "noise" words, making training feasible at scale.
