...
> #### Q1 — Skill 与静态文档的本质区别
> 
> Wiki 上的 API 设计指南是静态文本，不含触发条件、执行流程、输出约束，也不能自动校验质量，只能等人去查阅。而 Skill 通过 `description` 定义语义入口、在正文中规定执行步骤、通过模板约束输出格式、可限制可调用工具范围，甚至能通过 hooks 在完成后自动验证——它是"可被调用的行为模式"，而非参考资料。
> 
> #### Q2 — Skill 的两种触发方式
> 
> Skill 支持斜杠命令（如 `/review`）和 Claude 自动语义判断两种触发方式。前者适合用户明确知道自己要调用哪个能力的场景，后者适合用户只描述需求而无需知道 Skill 存在的场景；双向触发让两种场景都能被满足，同时保持能力定义的统一。
> 
> #### Q3 — 参考型与任务型 Skill 类比
> 
> 参考型 Skill 对应企业的"行为规范层"（世界规则），定义"正确的做事方式"，由模型根据语义自动加载，塑造行动方式而不主导行动；任务型 Skill 对应"操作流程层"（世界事件），定义一次明确的有边界行动（如部署、发布），因具有风险通常需要显式触发，常配合 `disable-model-invocation` 使用。
