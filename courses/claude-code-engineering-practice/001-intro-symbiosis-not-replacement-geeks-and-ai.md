...
1. 主代理是指挥官，子代理是专业兵种——分别负责代码审查（只读不改）、跑测试（执行汇报）、分析日志（提炼结论）。职责拆分是为了避免单一 Agent 角色混乱和上下文污染。
2. 传统工具需要用户手动输入 `/review` 触发；Skills 则让 AI 理解自然语言意图后自动判断任务类型，并自动激活对应的领域知识和检查清单，无需显式命令。
3. 目标是从 Claude Code 的使用者成长为"AI 工程指挥者"；课程聚焦两个痛点：Memory（Agent 每次对话从零开始、不理解项目背景）和 Sub-Agents（单一 Agent 角色混乱、上下文污染）。
