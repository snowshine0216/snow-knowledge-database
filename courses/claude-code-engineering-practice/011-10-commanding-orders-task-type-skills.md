...
> 
> #### Q1 — 任务型 vs 参考型 Skill 区别
> 
> 参考型 Skill 由 Claude 自主判断何时加载（按需调用），任务型 Skill 则通过 `disable-model-invocation: true` 关闭模型自动调用，确保只有用户显式输入斜杠命令才会触发执行，不会被 Claude 自行决定运行。
> 
> #### Q2 — 动态上下文注入解决问题
> 
> 模型启动时不知道当前运行环境的上下文（如当前所在分支、待合并的 commit 列表），`!command` 语法可在 Skill 加载时动态执行 shell 命令并将结果注入 Prompt，让模型获得真实的运行时上下文信息。
> 
> #### Q3 — 四大设计原则与权限最小化
> 
> 四个原则为：单一职责（一个命令只做一件事）、清晰命名（命令名直接表达其功能）、有意义的参数提示（让使用者了解如何传参）、权限最小化（严格控制每个任务的权限边界，即每个命令只拥有完成自身任务所需的最低权限，避免过度授权带来的安全风险）。
