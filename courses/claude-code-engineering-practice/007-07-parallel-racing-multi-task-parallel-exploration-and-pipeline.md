...
1. 并行探索的隐含前提是：各子代理的任务之间不能有信息依赖。电商案例中，auth/db/api 三个子代理分别发现了 JWT 中的 role、users 表中的 role 字段、以及 /admin/* 路由的 role 检查，但因并行执行互不知情，role 的完整传递路径无法被任何单个子代理还原——跨模块的"连点成线"必须由主对话承担。
2. 只读工具（Read/Grep/Glob）确保探索子代理不会意外修改代码，职责边界清晰；选用 haiku 是因为探索任务相对简单，haiku 速度更快、成本更低，符合"追求速度"的工程目标。
3. 并行探索适用于需要同时从多个独立角度理解一件事的场景（如拆解大型项目各模块），各子代理互不依赖；流水线编排适用于复杂任务可拆成多个连续阶段的场景（如修复 bug 时搜索→分析→修复→验证），前一阶段的输出是下一阶段的输入。
