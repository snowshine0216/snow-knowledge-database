...
> #### Q1 — 渐进式披露三层架构
> 
> 三层结构对应图书馆的"目录—章节—附录"：第一层是 description（系统扫描阶段只读这部分，作为目录判断相关性）；第二层是 SKILL.md 主文件（Skill 被触发时加载，作为章节）；第三层是按需引用的具体资源文件（作为附录，只在需要时加载）。
> 
> #### Q2 — 路由器与契约式引用
> 
> SKILL.md 通过 Quick Reference 表格，根据用户请求类型将 Claude 导向不同资源文件，充当路由器；契约式引用不只写一个文件路径，还明确标注"何时该加载这个文件"以及"加载后能得到什么"，让 Claude 拥有加载决策依据，而非盲目按路径读取。
> 
> #### Q3 — Knowledge ROI 与 80/20 决策
> 
> Knowledge ROI 指以最小 token 投入获得最大任务完成质量，因此要避免无关内容占用上下文；80/20 法则指 80% 的用户请求只需要 20% 的内容，所以高频、核心内容直接内联在主文件，低频、详细的参考信息外链到引用文件延迟加载。
