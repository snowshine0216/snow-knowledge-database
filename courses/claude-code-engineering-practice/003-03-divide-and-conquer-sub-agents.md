...
1. 主对话就是"老板"，每次让 Claude 亲自执行任务（跑测试、搜索、分析）都会把大量中间过程输出塞入同一上下文，导致重要信息被淹没。子代理拥有独立上下文窗口，执行完后只把结论摘要返回主对话，中间噪声随子代理上下文一起丢弃，主对话保持干净。
2. 三大价值是**隔离、约束、复用**。"约束"解决的是行为不可控问题——通过为子代理设定精确的工具权限边界，把"不希望你这么做"变成"物理上做不到"，例如代码审查子代理只读、修 bug 子代理才可写，职责边界不再依赖提示词的"自觉"。
3. Explore 专注快速只读搜索，负责"翻项目、找位置"；Plan 在修改代码前收集上下文、梳理依赖、生成实施路径；General-purpose 是能探索、能修改、能多步骤协作的全能型代理。分角色设计的原因是：让每个子代理只把高噪声过程留在自身内部，主对话只保留决策信息。
