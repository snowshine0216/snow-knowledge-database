...
1. 思维方式是：先识别痛点（AI 的某个行为让你不满意），再分析根因（权限过宽/过窄、职责模糊等），然后反推出子代理需要哪些能力限制和配置。课程强调这个思考过程比任何具体配置文件都更重要。
2. auth.js 的问题分三类：①硬编码密钥（SECRET_KEY/API_KEY），导致仓库泄露时密钥无法轮换；②弱密码逻辑（仅检查长度、明文比较、用可预测数据生成 token），使暴力破解和伪造身份成为可能；③信息泄露（登录失败时区分"用户不存在"与"密码错误"、接口返回含密码字段、eval 执行用户输入），大幅降低攻击成本。
3. `description` 字段需同时说明"做什么"（审查代码质量、安全、最佳实践）和"什么时候用"（代码修改后主动调用），这样 Claude 在协调多个子代理时才能正确判断何时委派任务给该审查器。
