...
1. 你认为 Sub-Agents（子代理）的核心设计思想是什么？它和普通的单一 AI 助手有何本质区别？
2. 在 AI Coding 工具大量涌现的背景下，Claude Code 最有可能以哪个差异化特性与其他工具区分？
3. 随着 AI 能力不断增强，程序员的角色定位会如何演变？"极客"这个词在 AI 时代有什么新的含义？
...
#### 主题2
- 原始小节标题：Another AI Coder？—— Claude Code 远不止如此
...
#### 主题3
- 原始小节标题：当 Sub-Agents 和 Skills 成为通用语言
...
## 关键要点
- 你好，我是黄佳。
- 主题2：2024 年末到 2025 年初，AI Coding 领域迎来了一次集中爆发。过去的2025年，我们已经见证过一轮又一轮 AI Coding 工具的浪潮。
- 主题3：这里我们必须单独强调 Sub-Agents 和 Skills 这两个概念，因为它们正在从 Claude Code 的原生技术术语泛化出来，演变成 AI 工程领域的通用语言（这个过程非常像是我们常说的“出圈”）。
...
1. 用自己的话解释 Sub-Agents 的"组织架构"比喻——主代理和子代理分别扮演什么角色，职责如何划分？
2. Skills 与传统工具触发方式（如手动输入 `/review`）的本质区别是什么？举例说明 Skills 如何做到"自动判断并激活"。
3. 黄佳为什么说这门课的目标是让你成为"AI 工程团队的指挥者"而非 Claude Code 的普通使用者？课程围绕哪几个核心工程痛点展开？

<details>
<summary>答案指南</summary>

1. 主代理是指挥官，子代理是专业兵种，各有明确职责与受限权限——例如负责代码审查的子代理只读不改，负责跑测试的子代理只执行并汇报结果，实现关注点分离。
2. 传统工具需要用户手动触发指令，而 Skills 让 AI 能自动理解意图——当你说"帮我看看这段代码有没有安全问题"，AI 自动判断这是安全审查任务并激活对应 Skill，无需手动选择。
3. 课程从真实工程卡点出发而非功能讲解，核心围绕 Memory（解决 Agent 无记忆、不理解项目背景）和 Sub-Agents（解决角色混乱、上下文污染）等问题，目标是将 Claude Code 打造成可持续运转的 AI 工程团队。
