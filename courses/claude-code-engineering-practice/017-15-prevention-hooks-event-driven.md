...
1. 控制点（PreToolUse 等）能阻止操作，因为动作尚未发生；接管点（PermissionRequest）替代人工决策，自动批准或拒绝权限弹窗；观察点（PostToolUse 等）只能记录和反馈，无法撤销已写入磁盘的文件。这种不对称设计避免了拦截后造成不一致状态。
2. PreToolUse 可以：允许（allow，放行）、拒绝（deny，拦截）、修改（updatedInput，改写参数后再执行）。例如将 `rm -rf /tmp/test` 改写为 `rm -rf /tmp/test --dry-run`，先预览删除结果再决定是否真正执行。
3. 用户级存放个人习惯（如日志格式、通知方式），项目级存放团队约定（如格式化规则、敏感文件列表）并提交到 git 让全员共享，本地覆盖用于临时调试。敏感文件保护属于团队安全约定，必须对所有成员生效，因此应提交到项目级配置。
