...
> #### Q1 — 两层能力体系划分与意义
> 
> 第一层"知识工程"（第9-11讲）解决知识管理问题——让 AI 在需要时精准获取专业知识而不浪费上下文；第二层"系统集成"（第12-14讲）解决系统设计问题——理解 Skills 与 SubAgents、Hooks、Plugins 如何组成完整工程体系，知道"放在哪里、和谁配合"才是真正能力。
> 
> #### Q2 — 四种设计模式与案例组合
> 
> 四种模式为：模板驱动（约束输出格式）、脚本增强（封装确定性逻辑）、分层知识（按频率分级加载）、工具隔离（权限是最重要的设计决策）；案例中第一步用"模板驱动+分层知识"建立规范（命名时只加载相关规范文件），第二步用"脚本增强+任务型"固化标准流程，四种模式不互斥，生产级 Skill 通常组合多种。
> 
> #### Q3 — Skills 终极定位与三大属性
> 
> Skills 是 Claude Code 架构中的**知识层**，把人类组织积累的知识管理经验技术化映射到 AI Agent 架构；与 Tools 的关系是"共生：约束、编排、反哺"，与 SubAgents 的关系是"互补：知识注入 vs 任务委托"；三个本质属性为：声明式（纯 Markdown 无运行时依赖）、自包含（一个目录完整交付）、知识本位（价值在内容不在格式）。
