...
> #### Q1 — 信噪比决策框架判断标准
> 
> 判断标准是信噪比：如果一个命令输出超过 50 行，且你只关心其中不到 10 行（不到五分之一），就应该用子代理。例如 `npm test` 输出 300 行，真正有用的只有"哪个测试失败、为什么"这一句结论，符合该标准。
> 
> #### Q2 — Haiku 模型选型工程逻辑
> 
> 测试运行器的任务是固定流程（执行命令）、模式匹配（解析输出）、模板填充（生成报告），这三类任务 Haiku 完全胜任，选 Haiku 更快更便宜；只有需要复杂推理的任务才值得用 Sonnet。
> 
> #### Q3 — 子代理隔离噪声的量化价值
> 
> 主对话 token 数从 8,800 降到 3,700，降幅 58%。更重要的收益是：后续每轮对话都不需要反复携带测试输出的噪声，避免上下文膨胀导致 Claude 注意力被稀释、回答质量下降。
