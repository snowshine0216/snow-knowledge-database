...
1. 在 AI 工程实践中，"噪声"指的是什么？当你让 Claude Code 执行 `npm test` 时，哪些输出内容可能属于"噪声"？
2. 如果测试输出有 300 行，但你只关心其中 1~2 行结论，这会对主对话的上下文产生什么影响？请凭直觉猜测。
3. 子代理（Sub-agent）和主对话直接执行命令相比，你认为主要区别在哪里？它为什么适合处理"高噪声"场景？
...
1. 用自己的话解释"信噪比决策框架"的具体判断标准，并举一个符合"应该用子代理"条件的实际例子。
2. 课程中测试运行器子代理选择了 Haiku 而非 Sonnet 模型，请解释这个决策背后的工程逻辑——什么样的任务特征决定了模型选型？
3. 课程用具体数字说明了子代理隔离噪声的价值：主对话 token 数从多少降到多少，降幅是多少？为什么说"节省 token"还不是最重要的收益？

<details>
<summary>答案指南</summary>

1. 判断标准是信噪比：如果一个命令输出超过 50 行，且你只关心其中不到 10 行（不到五分之一），就应该用子代理。例如 `npm test` 输出 300 行，真正有用的只有"哪个测试失败、为什么"这一句结论，符合该标准。
2. 测试运行器的任务是固定流程（执行命令）、模式匹配（解析输出）、模板填充（生成报告），这三类任务 Haiku 完全胜任，选 Haiku 更快更便宜；只有需要复杂推理的任务才值得用 Sonnet。
3. 主对话 token 数从 8,800 降到 3,700，降幅 58%。更重要的收益是：后续每轮对话都不需要反复携带测试输出的噪声，避免上下文膨胀导致 Claude 注意力被稀释、回答质量下降。
