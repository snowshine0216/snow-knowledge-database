...
> 
> #### Q1 — Skills 知识层 vs 工具层与智能体层
> 
> Tool Layer 是原子能力执行接口（类似 syscall），Skills 所在的 Knowledge Layer 是"在什么上下文下以什么步骤调用哪些工具"的策略规约层，Agent Layer 则是负责任务拆解与多单元协作编排的执行层——三层分别对应"能做什么""怎么决策""谁来执行"。
> 
> #### Q2 — Skills 三级进化路径各解决什么
> 
> 第一级 SOP 解决"个人经验无法复用"的问题；第二级专家系统解决"流程无法扩展为领域能力"的问题；第三级组织智能解决"单点能力无法规模化协同"的问题——对应从可执行→可扩展→可规模化的工程演进。
> 
> #### Q3 — 四种 Skill 设计模式核心本质
> 
> 模板驱动的本质是把自然语言生成转化为结构化接口（解决输出不稳定）；脚本增强的本质是把概率型推理替换为确定性执行（解决结果不稳定）；知识分层的本质是通过渐进加载控制认知复杂度（解决上下文膨胀）；工具隔离的本质是把安全约束前置为结构设计（解决越权风险）。
