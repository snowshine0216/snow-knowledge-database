...
1. Agentic Loop 分三个阶段灵活切换：Claude 调用工具→获得返回结果→结果反馈给 Claude 影响下一步决策，如此循环。简单任务一轮即止，复杂任务可循环几十次。

2. 涌现的三要素是：原语工具提供行动能力（感知/搜索/修改/执行/获取）、LLM 推理提供决策能力（理解→分解→选择→解读）、反馈循环让二者持续交互。没有任何单一工具能"修复 bug"，但三者组合就能完成这类复杂任务。

3. Read/Grep/Glob 属只读低风险操作无需授权；Bash 能力最强也最危险，始终需要用户授权。实践上，只读任务的子代理只需配 Read/Grep/Glob 最小权限，高风险工具应加 Hook 检查，这样既保安全又减少不必要的权限弹窗。
