---
tags: [claude-code, geektime, ai-agents, tools, internals]
source: https://time.geekbang.org/column/article/957210
---

## Pre-test

> *阅读前尝试回答以下问题。答错完全正常——预测试能让大脑在接触正确答案时编码得更深。*

1. 你认为一个开发者每天的工作，可以被分解为哪几种最基本的"原子操作"？请列举你能想到的。
2. Claude Code 内置了十几个工具（Read、Write、Bash、Grep 等），为什么不设计"重构工具"或"调试工具"这类高级专用工具，而只提供这些看起来很基础的工具？
3. 在工具权限管理中，你认为哪类操作风险最高、必须要用户确认？哪类风险最低、可以直接执行？

---

# 18｜庖丁解牛：Tools 工具系统内核剖析

## 章节元数据
- 课程名（EN）: claude-code-engineering-practice
- 课程名（ZH）: Claude Code 工程化实战
- 章节: 
- 作者: 黄佳
- 文章 ID: 957210
- 来源: https://time.geekbang.org/column/article/957210

## 康奈尔笔记

### 线索栏（问题）
- 本章的核心论点是什么？
- 本章开场提出了哪些背景、问题与动机？
- “主题2”这一部分的关键观点是什么？
- “主题2”有哪些支撑证据或案例？
- “主题3”这一部分的关键观点是什么？
- “主题3”有哪些支撑证据或案例？
- “工具的风险等级”这一部分的关键观点是什么？
- “工具的风险等级”有哪些支撑证据或案例？

### 笔记栏
#### 概览
- 释题：庖丁解牛。庖丁之所以神，不是刀快，而是他知道牛的结构——骨节之间、筋络之处，刀刃游走其间，从无阻碍。理解 Claude Code 的工具系统也是一样：不需要记住每一个工具的参数，而是理解工具背后的设计哲学：五个原子操作、涌现的原理、权限的层次。理解了结构，用起来才能游刃有余。
- 你好，我是黄佳。
- 上一讲我们学习了 MCP，通过开放协议将 Claude Code 连接到外部世界。GitHub、Notion、数据库……MCP 让 Claude 的能力边界从本地文件系统扩展到了整个数字世界。
- 但有一个更根本的问题我们一直没有深入讨论：Claude Code 内置的那些工具本身是怎么回事？ Read、Write、Bash、Grep、Glob、Edit、Agent……这十几个“原生工具”是怎么组织的？为什么偏偏是这几个？它们和 MCP 扩展的外部工具又是什么关系？
- 这一讲，我们庖丁解牛，把 Claude Code 的工具系统彻底拆开来看。按照重要程度，这一讲本应放在基础篇，不过，因为目前大家知识体系已经很全面了，我还是决定放在这里，我们理解起来更深入。

#### 主题2
- 原始小节标题：Agentic Loop——工具驱动的智能循环
- 如果 Claude 没有工具，它只能做一件事：输出文本。
- 它能思考、能分析、能给建议，但不能行动。
- 有了工具，情况完全不同，我们和Claude的交流可能是这样的。
- 工具让 Claude 从顾问变成了执行者。
- Claude Code 的工作循环——Agentic Loop 可以分为三个阶段，根据任务需要灵活切换。
- 简单任务可能只需要一轮，复杂任务可能循环几十次。每一次工具调用的返回结果，都会反馈给 Claude，影响下一步的决策。

#### 主题3
- 原始小节标题：Claude Code 内置工具完整清单
- 早期，Claude Code 只是内置了十几个工具（目前逐渐增多，可能已经有20多个了，最新列表在这里），按功能分为六个类别。
- 文件操作类，Claude 与代码交互的最基本方式。Read 是只读的，不需要额外授权；Edit、Write、MultiEdit 会修改文件系统，因此需要用户确认。
- 搜索类，Claude 的“眼睛”——在动手之前先看清楚代码库的全貌。Glob 按文件名模式定位文件，Grep 按内容搜索定位代码行，两者都是只读操作，无需授权。
- 执行类，整个工具集里能力最强也最危险的一个。Bash 可以执行操作系统能做的一切。正因如此，它始终需要用户授权。
- 网络类，让 Claude 突破本地文件系统的边界，访问互联网上的信息。查文档、搜报错、获取 API 响应都靠这两个工具。
- 编排类，这组工具不直接操作代码，而是管理 Agent 的工作流程：把复杂任务委派给子代理、在需要时向用户提问、用任务清单跟踪多步骤进度。

#### 工具的风险等级
- 为什么要这样分类？因为不同类别的工具有不同的风险等级：
- 理解这个分类，你就能：
- 为子代理配置最小权限（只读任务只给 Read/Grep/Glob）
- 设计防护机制（高风险工具加 Hook 检查）
- 理解权限提示的逻辑（低风险工具不弹窗，高风险工具要确认）

#### 从原语到涌现——工具设计哲学
- 看完工具清单，你可能会问：只有十几个内置工具，凭什么覆盖几乎所有软件工程任务？为什么没有看到重构工具、调试工具、部署工具这些高级工具？
- 答案指向同一个工程哲学：Claude Code 的工具集是一组精心选择的原语（primitives），而不是一组特化的高级能力。

#### 软件工程的五个原子操作
- 一个开发者每天做的所有事情，无论多么复杂，都可以分解为五种原子操作。
- 想想你的一天都会哪些动作？
- 你打开文件看代码 → 感知
- 你搜索某个函数在哪里被调用 → 搜索
- 你修改代码修复一个 bug → 修改
- 你运行 npm test 验证修复 → 执行

#### 类比：为什么编程语言只需要几条指令
- 这个设计哲学并不新鲜，我们回顾一些优秀的产品，就能发现它们的思路都是这样。
- Unix 的设计哲学是“做好一件事”（Do One Thing Well），然后通过管道组合。Claude Code 的工具设计完全继承了这个思想，但比 Unix 管道强大得多。Unix 管道是预定义的、线性的。而 Claude Code 的组合是动态的、分支的、自适应的，Claude 在每次工具调用后，根据返回的结果重新推理下一步该做什么。

#### 涌现：为什么 1+1 远大于 2
- 为什么这些简单工具组合在一起，能产生远超其各自能力之和的效果？
- 答案是涌现（Emergence）：
- 原语工具提供行动能力（感知、搜索、修改、执行、获取）
- LLM 推理提供决策能力（理解任务 → 分解步骤 → 选择工具 → 解读结果）
- 反馈循环让二者持续交互（工具输出 → 新的理解 → 新的行动）
- 我们用一个具体例子说明。比方说，我们要完成“修复用户登录时偶尔出现的 500 错误”这任务，虽然没有任何一个工具能“修复 bug”，但看看它们是怎么组合的。

### 总结
这一讲，我们庖丁解牛，把 Claude Code 的工具系统彻底拆开来看，探讨一下它的设计哲学。

## 关键要点
- 释题：庖丁解牛。庖丁之所以神，不是刀快，而是他知道牛的结构——骨节之间、筋络之处，刀刃游走其间，从无阻碍。理解 Claude Code 的工具系统也是一样：不需要记住每一个工具的参数，而是理解工具背后的设计哲学：五个原子操作、涌现的原理、权限的层次。理解了结构，用起来才能游刃有余。
- 主题2：如果 Claude 没有工具，它只能做一件事：输出文本。
- 主题3：早期，Claude Code 只是内置了十几个工具（目前逐渐增多，可能已经有20多个了，最新列表在这里），按功能分为六个类别。
- 工具的风险等级：为什么要这样分类？因为不同类别的工具有不同的风险等级：
- 从原语到涌现——工具设计哲学：看完工具清单，你可能会问：只有十几个内置工具，凭什么覆盖几乎所有软件工程任务？为什么没有看到重构工具、调试工具、部署工具这些高级工具？
- 软件工程的五个原子操作：一个开发者每天做的所有事情，无论多么复杂，都可以分解为五种原子操作。

## 知识图谱种子
- 实体:
  - claude-code-engineering-practice
  - 18｜庖丁解牛：Tools 工具系统内核剖析
  - 黄佳
- 关系:
  - (claude-code-engineering-practice) -> 包含章节 -> (18｜庖丁解牛：Tools 工具系统内核剖析)
  - (18｜庖丁解牛：Tools 工具系统内核剖析) -> 作者 -> (黄佳)

## 复习备注
- 构建知识图谱前，先复核关键论断与原文的一致性。
- 在此补充你的行动项、实践映射和复盘结论。


---

## Post-test

> *关闭文件，凭记忆写出或大声说出你的答案，再对照答案指南（费曼检验：无法简单解释，说明仍有理解空白）。*

1. 用自己的话解释 Claude Code 的 Agentic Loop 是如何运作的——工具调用的结果对 Claude 的下一步决策有什么影响？
2. 本章用"涌现"（Emergence）来解释为什么简单工具组合能完成复杂任务。请用自己的话说明涌现的三个要素，并举例说明它们如何协作完成"修复登录 500 错误"这类任务。
3. Claude Code 将内置工具按风险等级分类，并据此设计权限机制。请解释这套分类逻辑对实际使用有什么指导意义——比如为子代理配权限时，你会怎么做？

> [!example]- Answer Guide
> 
> #### Q1 — Agentic Loop 运作机制
> 
> Agentic Loop 分三个阶段灵活切换：Claude 调用工具→获得返回结果→结果反馈给 Claude 影响下一步决策，如此循环。简单任务一轮即止，复杂任务可循环几十次。
> 
> #### Q2 — 涌现的三个要素
> 
> 涌现的三要素是：原语工具提供行动能力（感知/搜索/修改/执行/获取）、LLM 推理提供决策能力（理解→分解→选择→解读）、反馈循环让二者持续交互。没有任何单一工具能"修复 bug"，但三者组合就能完成这类复杂任务。
> 
> #### Q3 — 工具风险分级与权限配置
> 
> Read/Grep/Glob 属只读低风险操作无需授权；Bash 能力最强也最危险，始终需要用户授权。实践上，只读任务的子代理只需配 Read/Grep/Glob 最小权限，高风险工具应加 Hook 检查，这样既保安全又减少不必要的权限弹窗。
