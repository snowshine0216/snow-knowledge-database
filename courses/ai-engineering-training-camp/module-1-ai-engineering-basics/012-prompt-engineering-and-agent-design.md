...
> #### Q1 — Agent 三层记忆系统
> 
> **L1（Working Memory）**：存当前对话历史，保存在上下文窗口，对话结束即释放。
> 
> **L2（Session Memory）**：存 Session ID 和用户画像，存于 Redis/DynamoDB，跨设备可用。
> 
> **L3（Long-term Memory）**：存用户偏好和历史行为，存于向量 DB，跨会话持久。
> 
> 多人混用同一 Session ID 会导致不同用户的画像相互污染（如孩子问作业与成人问工作的画像被合并），Agent 表现"时好时坏"。
> 
> ---
> 
> #### Q2 — 槽位填充三类区别
> 
> - **必填槽**：缺失时必须追问用户，否则流程无法继续。
> - **可选槽**：缺失时提供默认值，不追问。
> - **依赖槽**：只在前置槽全部填完后才激活推送。
> 
> 机票示例：出发城市 → 到达城市 → 航班时间三个必填槽依次填完后，才解锁"推荐目的地酒店"和"推荐旅游套餐"两个依赖槽。
> 
> ---
> 
> #### Q3 — 生产级四级降级策略
> 
> 顺序：**Level 1 重试**（指数退避）→ **Level 2 降级模型**（切换低延迟模型，牺牲质量保持智能）→ **Level 3 规则引擎**（模型完全失效时用规则匹配保证任务完成）→ **Level 4 转人工**（最终兜底）。
> 
> 必须设计转人工是因为用户不能被无限"保持"，服务连续性要求任务最终必须能完成。
