...
> 
> #### Q1 — Tool Calling Five-Step Flow
> 
> 模型只输出 JSON 指令（`finish_reason = "tool_calls"`）来告知客户端调用哪个工具，工具的实际执行由客户端在本地完成，结果再追加到消息历史后发回模型生成最终答案。工具在客户端执行是安全边界的核心设计——控制权始终在开发者手中。
> 
> #### Q2 — Function Calling vs MCP Tradeoffs
> 
> Function Calling 属于应用层（安全性高、稳定、控制权在开发者，适合内部 API/数据库），MCP 属于协议层（灵活、可从服务器动态获取工具，适合外部 SaaS 服务，但存在第三方安全风险和稳定性较低的问题）。
> 
> #### Q3 — Tool Definition Best Practices
> 
> 核心实践包括：函数名清晰避免缩写、用 `enum` 约束有限值参数、控制工具数量不超过 20 个。"已知参数不让模型填充"的原因是模型在填充确定性信息（如当天日期）时容易出错，应由代码直接获取，只让模型从用户输入中提取真正未知的参数。
