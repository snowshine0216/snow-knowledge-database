...
> 
> #### Q1 — Function Calling 完整工作流程
> 
> 流程分两次独立交互：第一次请求模型判断是否需要工具，模型返回的是 `tool_call JSON`（不是最终答案）；客户端拿到 JSON 后在本地执行工具函数，再将执行结果追加到消息历史，第二次请求模型才得到最终答案。两步设计的目的是解耦"要不要调用"（模型负责）和"具体怎么执行"（函数负责），工具始终在客户端执行，不在模型端。
> 
> #### Q2 — load_dotenv 冲突默认行为
> 
> `load_dotenv()` 默认**不覆盖**系统中已存在的同名环境变量；若需要强制覆盖，需传入参数 `override=True`。
> 
> #### Q3 — Function Calling 扩展性优势
> 
> 硬编码方案改需求就必须改代码，随功能增长维护成本线性上升；Function Calling 只需增量添加或修改工具定义，模型只负责决策"要不要调用"，具体实现完全由独立函数承担，两者彻底解耦，业务逻辑稳定，扩展几乎不影响已有代码。
