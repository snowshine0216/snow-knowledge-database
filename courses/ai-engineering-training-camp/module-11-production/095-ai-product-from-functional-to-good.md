...
1. 数据层附加置信度、证据来源和错误码；交互层根据置信度区间展示不同提示（>0.8直接显示、0.5–0.8加"仅供参考"、<0.5转人工客服）；架构层在超时或崩溃时依次降级为规则引擎或静态页面，三层各司其职、互为兜底。
2. DSL 用可视化工作流（如 Dify）将业务逻辑显性化，业务方能看懂、工程师可直接转换为状态机代码，需求变更只改 DSL 不动核心代码；由此避免"过度设计"（先搭微服务两周没跑通）和"需求反复横跳"（如 AIoT 收货柜上线前临时叠加优惠券功能导致工程师崩溃）两大 MVP 陷阱。
3. 正向信号包括：复制 AI 答案、分享 AI 回答、在答案页停留超 30 秒；负向信号包括：点击"重新生成"、手动修改 AI 输出后继续对话。这些数据定期用于微调模型或更新知识库，使模型越来越懂当前用户场景，体验提升后用户自发传播，吸引更多用户产生更多数据，形成正向飞轮。
