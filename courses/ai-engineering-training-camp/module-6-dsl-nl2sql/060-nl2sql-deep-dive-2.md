...
> #### Q1 — 78.9% 准确率为何不可接受
> 
> 78.9% 意味着约每 5 条 SQL 就有 1 条出错，涉及写操作时会造成数据错误，生产环境不可接受。微调最适合"AI 取数"场景——简单查询、常见聚合、用户频繁重复的问题，而非多表联合查询或陌生查询。
> 
> #### Q2 — AI 取数数据飞轮机制
> 
> 用户确认某条 SQL 正确后，该 SQL 回流向量数据库；下次相同或相似问题被提出时，系统直接从向量库检索成功案例，大模型参考生成，命中率随使用量持续上升，形成数据飞轮。
> 
> #### Q3 — DB-GPT-Hub 与 LlamaFactory 核心差异
> 
> DB-GPT-Hub 专为 Text2SQL 设计、配置简单，仅支持 ALPACA 格式；更换模型时必须修改 LoRA Target（如 Qwen 需改为 `c_attn`）。LlamaFactory 支持最新模型和 WebUI 图形化操作，额外支持 ShareGPT 格式；更换数据集时必须检查 JSON 格式是否匹配，这是最常见出错点。
