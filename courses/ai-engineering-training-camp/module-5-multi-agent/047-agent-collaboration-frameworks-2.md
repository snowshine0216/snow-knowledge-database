...
1. 在多 Agent 系统中，"Round-Robin（轮循）"调度方式是如何决定哪个 Agent 下一个发言的？
2. Swarm 模式与传统中心化调度相比，最根本的架构区别是什么？
3. 如果想让 AI Agent 在执行过程中暂停并等待人类输入意见，你认为应该如何实现？
...
1. 用自己的话解释 `RoundRobinGroupChat` 的工作方式，以及"创作者与评论家"案例中终止条件是如何触发的？
2. Swarm 模式中 Agent 之间如何传递控制权？结合航班退票案例，说明整个流程如何在没有中央调度节点的情况下完成任务。
3. 在 AgentChat 中如何实现 Human-in-the-Loop？`UserProxyAgent` 的 `input_func` 参数起什么作用，前端集成时又可以用什么方式替代命令行输入？

> [!example]- Answer Guide
> 
> #### Q1 — RoundRobinGroupChat 与终止条件
> 
> `RoundRobinGroupChat` 让所有参与者按列表顺序依次发言；在"创作者与评论家"案例中，`TextMentionTermination("APPROVE")` 作为终止条件，当 Critic Agent 的回复中出现 `APPROVE` 时对话立即结束，不必等待固定轮数跑完。
> 
> #### Q2 — Swarm 去中心化控制传递
> 
> Swarm 模式中每个 Agent 通过发送 `HandoffMessage` 自主决定将控制权移交给谁，无需独立的中央调度节点；在退票案例里，TravelAgent 先评估请求，信息不足则移交给 User 追问，信息明确则移交给 FlightRefundAgent 完成退款，退款后再移交回 TravelAgent 触发终止——整个流程由各 Agent 的 `system_message` 提示词驱动，无硬编码分支。
> 
> #### Q3 — Human-in-the-Loop 与前端集成
> 
> 通过将 `UserProxyAgent` 的 `input_func` 设为 Python 内置的 `input()` 函数，流程在该 Agent 轮次时会暂停并等待命令行输入；在前端集成场景中，AgentChat 支持 WebSocket，可以用 ws 方式替代 `input()`，让前端页面实时接收并发送交互内容。
