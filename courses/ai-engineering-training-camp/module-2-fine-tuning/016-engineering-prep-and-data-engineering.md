...
> 
> #### Q1 — Docker、Tokenizer、DVC 三件事工程风险
> 
> Docker 解决环境不一致（Python 版本/依赖冲突），用 `docker build` 锁死运行环境；Tokenizer 一致性验证确保训练和推理使用同一个分词器（BPE/WordPiece 错配会导致模型表现断崖式下滑）；DVC 保证不同环境使用完全相同的数据集（用 MD5 指纹验证），防止数据被"顺手更新"后无法复现训练结果。三件事任何一件缺失，上线都有大概率出问题。
> 
> #### Q2 — 微调效果退步是正常现象
> 
> 微调是在有限数据上调整部分参数，容易过拟合或破坏原有泛化能力，因此效果退步是正常现象而非失败信号。真正的挑战在于：建立快速定位问题（工程环境/数据/超参数）的能力，以及通过用户反馈→问题追踪→迭代改进的反馈渠道持续优化，而不是把"跑起来"当作终点。
> 
> #### Q3 — DVC 化解数据认知冲突
> 
> 数据工程师认为数据应"实时最新"，而微调工程师要求训练/测试/生产必须使用同一份数据（只有一个变量）。DVC 通过 `dvc add` 将数据纳入版本管理、`dvc push/pull` 同步到远程存储，使任何成员都能用 `git pull + dvc pull` 还原完全相同的数据；需要更新数据时创建新版本而非覆盖，旧实验永远可用 `dvc checkout` 一键复现。
