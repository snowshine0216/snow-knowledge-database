...
1. LoRA 不直接更新预训练权重 W，而是添加低秩旁路 ΔW = A × B（W 冻结，仅训练 A 和 B）。以 d=1024、r=8 为例：全量参数 1024×1024=1,048,576，LoRA 参数 1024×8+8×1024=16,384，仅约 1.56%，显存节省超过 90%。
2. 梯度范数正常区间为 0.5–0.7；大于 5 表示梯度爆炸，小于 0.1 表示梯度消失。残差连接通过计算 output = x + F(x)，在反向传播路径中保留一条梯度直通通道（梯度始终 ≥ 1），避免多层叠加后梯度指数级衰减。
3. 必须先执行 `swift export --merge_lora true` 将 LoRA adapters 合并到基础模型权重中，因为 vLLM 不直接支持 Swift 格式的 LoRA checkpoints。分离模式灵活、存储小但推理有 5–10% 额外延迟，适合开发阶段；合并模式推理无额外开销但失去快速切换能力，适合生产部署。
