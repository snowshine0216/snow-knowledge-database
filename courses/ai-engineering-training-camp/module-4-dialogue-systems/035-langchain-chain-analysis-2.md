...
> #### Q1 — 三层降级 Fallback 方案
> 
> 第一层调用主模型（如 qwen-max），失败后第二层切换备用模型（如 qwen-plus 或其他供应商），两层均失败则第三层返回兜底响应（不调用任何模型，直接返回提示用户人工协助的 JSON）。
> 
> #### Q2 — try/except 替代 if/else 优势
> 
> `try/except` 让代码更线性，无需预先枚举所有失败场景；异常会自动携带错误信息便于日志记录，而大量 `if/else` 需要手动判断每种失败条件，逻辑繁琐且容易遗漏。
> 
> #### Q3 — 指数退避等待时间计算
> 
> 等待时间公式为 `base_delay * (2 ** attempt)`，即第 1 次失败等 1s、第 2 次等 2s、第 3 次等 4s；指数增长的设计意图是给上游服务足够的恢复时间，同时避免立即重试加重服务压力。
