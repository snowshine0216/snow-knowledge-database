...
1. 本节为代码演示课，讲师直接在 IDE 中展示，核心操作是在已有 `index` 对象之后追加新代码，以扩展索引功能。
2. 推断涉及"节点后处理器（Node Postprocessors）"和"自定义检索器"，前者用于对检索结果进行过滤或重排，后者允许定制检索逻辑以超越默认向量相似度搜索。
3. 由于 ASR 仅捕获约 12 行片段，技术细节无法完整重建；课程建议结合 028 笔记和前序课程的代码示例共同学习。
