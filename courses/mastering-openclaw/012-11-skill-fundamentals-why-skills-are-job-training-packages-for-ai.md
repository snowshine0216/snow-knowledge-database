...
> #### Q1 — Skills 本质与培训手册比喻
> 
> Skills 是"直接可用的封装好的 AI 能力包"，就像岗位培训手册——装上即用，不需要每次从头教，因为能力已被预先封装，可随时调用而无需重复配置。
> 
> #### Q2 — 三大痛点与效率影响
> 
> 三大痛点：**重复劳动**（每次要从头告诉 AI 如何工作，等同于从零造脚本）、**忘记偏好**（AI 可能在下次对话中忘掉用户的习惯和设置）、**上下文负担**（每次对话都要塞入大量背景信息，低效且耗时）。
> 
> #### Q3 — OpenClaw 安装与永久可用
> 
> 直接与龙虾（OpenClaw）对话即完成安装，例如说"帮我安装 Agent Rich 这个 Skill"，AI 会自主执行安装流程；安装后 AI 获得该 Skill 对应的工作能力（如自动抓取网络内容），无需每次手动配置，故称为永久可用。
